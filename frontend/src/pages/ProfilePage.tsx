...
/** A themed card with a colour-coded icon tile header — the platform's standard section shell. */
  ...
  return (
    <section
      ...
      style={{
        ...
        border: `1px solid ${danger ? 'rgba(239,68,68,0.22)' : 'var(--color-border)'}`,
      }}
      onMouseEnter={(e) => { if (!danger) e.currentTarget.style.borderColor = 'var(--color-border-hover)' }}
      onMouseLeave={(e) => { if (!danger) e.currentTarget.style.borderColor = 'var(--color-border)' }}
    >
      <div className="flex items-center gap-2.5 mb-5">
        ...
        <h2 className="text-sm font-semibold" style={{ color: danger ? '#f87171' : 'white' }}>{title}</h2>
...
function PlanBadge({ plan }: { plan?: string }) {
  const p = PLAN_LABELS[plan ?? 'free'] ?? PLAN_LABELS.free
  return (
    <span
...
export default function ProfilePage() {
  ...
  const heroStats = [
    ...
    { icon: Zap, label: 'Tokens', value: summary ? formatTokens(summary.totalTokens) : '—', color: '#3b82f6' },
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-10">
      ...
      >
        ...
        <div className="relative p-6 sm:p-8 flex flex-col md:flex-row md:items-center gap-6">
          ...
          <div className="relative flex-shrink-0 mx-auto md:mx-0">
            <div
              className="w-24 h-24 rounded-2xl flex items-center justify-center overflow-hidden shadow-lg"
            ...
            <button
              ...
              className="absolute -bottom-2 -right-2 w-9 h-9 rounded-xl flex items-center justify-center text-white transition-all hover:opacity-90 active:scale-95 disabled:opacity-50 shadow-md"
              style={{ background: 'var(--color-surface-3)', border: '1px solid var(--color-border-hover)' }}
            >
              <Camera size={15} />
            </button>
          ...
          <div className="flex-1 min-w-0 text-center md:text-left">
            <div className="flex items-center gap-2.5 justify-center md:justify-start mb-1.5">
          ...
          <div className="grid grid-cols-3 gap-3 flex-shrink-0">
            {heroStats.map(({ icon: Icon, label, value, color }) => (
              <div
                key={label}
                className="rounded-xl px-4 py-3 text-center md:text-left min-w-[88px]"
                style={{ background: 'var(--color-surface-2)', border: '1px solid var(--color-border)' }}
              >
                <div className="flex items-center gap-1.5 justify-center md:justify-start mb-1" style={{ color }}>
                  <Icon size={12} />
      ...
      {/* Two-column: identity/security · plan/integrations */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 items-start">
        {/* Left column */}
        <div className="flex flex-col gap-5">
          {/* Personal info */}
          <SectionCard icon={<User size={15} />} iconColor="#8b5cf6" title="Personal information">
          ...
          {/* Change password */}
          <SectionCard icon={<Lock size={15} />} iconColor="#3b82f6" title="Change password">
        ...
        <div className="flex flex-col gap-5">
          ...
          {billing ? (() => {
            ...
            return (
              <SectionCard
                ...
                iconColor="#10b981"
                title="Plan & Billing"
                badge={<PlanBadge plan={billing.plan} />}
              >
                ...
                <div className="flex flex-wrap gap-2 mt-5">
                  {upgrades.map((p) => (
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 rounded-xl px-4 py-2.5 text-sm font-medium text-white transition-all hover:opacity-90 active:scale-95 disabled:opacity-50"
                      style={{ background: 'linear-gradient(135deg, var(--color-brand-600), #3b82f6)' }}
                    >
                  ...
                  {billing.plan !== 'free' && (
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 rounded-xl px-4 py-2.5 text-sm font-medium transition-all hover:opacity-90 disabled:opacity-50"
          ...
          {/* Integrations */}
          <SectionCard icon={<Link2 size={15} />} iconColor="#f59e0b" title="Integrations">
      ...
      <div className="mt-5">
        <SectionCard
          ...
          badge={
            <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: 'var(--color-surface-3)', color: 'rgba(255,255,255,0.4)' }}>
              Last 30 days
            </span>
          }
        >
                    ...
                    <thead>
                      <tr style={{ background: 'var(--color-surface-2)', borderBottom: '1px solid var(--color-border)' }}>
                        {['Provider / Model', 'Agent', 'Tokens', 'Cost', 'When'].map((h) => (
                          <th key={h} className="text-left px-3 py-2.5 font-medium whitespace-nowrap" style={{ color: 'rgba(255,255,255,0.35)' }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {logs.map((log, i) => (
                        <tr key={log.id} style={{ background: i % 2 === 0 ? 'transparent' : 'var(--color-surface-2)', borderBottom: i < logs.length - 1 ? '1px solid var(--color-border)' : undefined }}>
                          ...
                          <td className="px-3 py-2.5 whitespace-nowrap" style={{ color: 'rgba(255,255,255,0.5)' }}>{log.agentId ?? '—'}</td>
                          <td className="px-3 py-2.5 whitespace-nowrap">
                          ...
                          <td className="px-3 py-2.5 whitespace-nowrap" style={{ color: '#10b981' }}>{formatCost(log.costUsd)}</td>
                          <td className="px-3 py-2.5 whitespace-nowrap" style={{ color: 'rgba(255,255,255,0.3)' }}>
      ...
      <div className="mt-5">
        <SectionCard icon={<ShieldAlert size={15} />} iconColor="#ef4444" title="Danger zone" danger>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <button
              ...
              disabled={deleting}
              className="sm:ml-6 flex-shrink-0 px-4 py-2 rounded-xl text-sm font-semibold transition-all disabled:opacity-50"
              style={{
